...
struct FinancialSummaryDetailView: View {
    ...
    private var overallSummarySection: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 15) {
                SummaryMetricCard(
                    title: "Total Revenue",
                    value: String(format: "%.2f", proposal.totalAmount),
                ...
                SummaryMetricCard(
                    title: "Total Cost",
                    value: String(format: "%.2f", proposal.totalCost),
                ...
                SummaryMetricCard(
                    title: "Gross Profit",
                    value: String(format: "%.2f", proposal.grossProfit),
    ...
    private var engineeringAnalysisSection: some View {
        VStack(alignment: .leading, spacing: 15) {
            ...
            if proposal.engineeringArray.isEmpty {
                ...
                let avgRate = proposal.subtotalEngineering / totalDays
                ...
                HStack {
                    ...
                    SummaryMetricCard(
                        ...
                        value: String(format: "%.2f", avgRate),
    ...
    private var keyFinancialIndicatorsSection: some View {
        VStack(alignment: .leading, spacing: 15) {
            ...
            VStack(spacing: 10) {
                KPIRow(title: "Avg Margin per Product", value: String(format: "%.2f", avgMarginPerProduct))
                KPIRow(title: "Avg Product Discount", value: String(format: "%.1f%%", avgDiscount))
                KPIRow(title: "Revenue per Category", value: String(format: "%.2f", revPerCategory))
                KPIRow(title: "Engineering % of Revenue", value: String(format: "%.1f%%", (proposal.subtotalEngineering / proposal.totalAmount) * 100))
                KPIRow(title: "Expenses % of Revenue", value: String(format: "%.1f%%", (proposal.subtotalExpenses / proposal.totalAmount) * 100))
                KPIRow(title: "Tax Rate", value: String(format: "%.1f%%", (proposal.subtotalTaxes / (proposal.totalAmount - proposal.subtotalTaxes)) * 100))
            }
        }
        .padding()
        .background(Color(UIColor.systemBackground))
...
struct EngineeringDetailRow: View {
    ...
    var body: some View {
        HStack {
            ...
            Text(String(format: "%.1f days @ %.2f", engineering.days, engineering.rate))
                .font(.caption)
                .foregroundColor(.secondary)
